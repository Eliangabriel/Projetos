...
class CaptureBotApp:
    def __init__(self, master):
        ...
        self.bot_token, self.chat_id, self.caminho_pasta, self.intervalo_captura, self.mensagem, self.capturar_tela_inteira, self.nome_executavel = self.ler_arquivo_config(self.CONFIG_FILE)
        ...
        with mss.mss() as sct:
            try:
                while self.captura_ativa:
                    ...
                    # Limpa a pasta a cada 10 envios
                    if self.envios_contador >= 10:
                        self.apagar_arquivos_antigos(self.caminho_pasta)
                        self.envios_contador = 0  # Reseta o contador após a limpeza

                    # Aguarda pelo intervalo de captura antes de fazer a próxima
    ...
    def ler_arquivo_config(self, caminho_arquivo):
        ...
        return bot_token, chat_id, caminho_pasta, intervalo_captura, mensagem, capturar_tela_inteira, nome_executavel
    ...
    def show_config(self):
        ...
        ttk.Label(config_window, text="Intervalo de Captura (segundos):").grid(row=2, column=0, padx=5, pady=5)
        self.intervalo_captura_entry = ttk.Entry(config_window)
        self.intervalo_captura_entry.insert(0, str(self.intervalo_captura))
        self.intervalo_captura_entry.grid(row=2, column=1, padx=5, pady=5)
    ...
    def salvar_config(self):
        ...
        self.intervalo_captura = int(self.intervalo_captura_entry.get())

        config = configparser.ConfigParser()
        config.read(self.CONFIG_FILE)
        config["Capturas"]["mensagem"] = self.mensagem.decode("utf-8")
        config["Capturas"]["caminho_pasta"] = self.caminho_pasta
        config["Capturas"]["intervalo_captura"] = str(self.intervalo_captura)
